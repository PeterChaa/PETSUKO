...
feedButton.addEventListener("click", function () {
    ...
    // Every feed advances the bar; every 3rd feed evolves and rolls the bar
    // back over (carrying any remainder) so it takes another full 3 feeds
    // to reach the next evolution instead of re-evolving on every click.
    ...
    // Show hearts GIF on every press
...
function evolveCharacter() {
    ...
    // Remove any previously evolved image(s) - the initial hidden placeholder
    // from character.js and/or a prior evolution's gif - so evolutions
    // replace each other instead of stacking on top of one another.
    document.querySelectorAll(".evolved").forEach(function (el) {
        el.remove();
    });
    ...
    document.getElementById("gameFrame").appendChild(evoElement);
